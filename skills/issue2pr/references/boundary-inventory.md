...
| `profile init` reaches the source system without a driver | assumption | **not** parameterised, and structurally cannot be: the driver is selected by `source_driver` in a profile, and this is the command that writes the profile. Recorded as the one pre-profile exemption in `gh_boundary_lint.py`, narrowed to two read-only probes. |
