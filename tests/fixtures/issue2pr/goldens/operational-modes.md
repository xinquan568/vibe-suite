...
- **Writes:** `round-<N+1>/`; the per-round override records `review_mode_overrides`,
  ...
  **`00-meta.json` is never rewritten** — it records how the run started, and a run that could edit
  its own origin story has no origin story.
...
- **Round bounds:** the cap **inherits**, and an override applies to the new round only, recorded in
  `max_review_rounds_overrides["<N+1>"]`. The domain and its clamp rule are the contract's and are
  not restated here — see [`SKILL.md` § Round bounds](../SKILL.md#round-bounds). **This is the
  documented remedy for a round that stopped `EXIT_MAX_ROUNDS`** — iterate at a higher cap rather
  than re-running into the same wall.
