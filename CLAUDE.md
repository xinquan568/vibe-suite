...
- **`codex/` is generated.** `MIRROR-MANIFEST.json` binds every file to its source bytes;
  hand-edits fail `--mirrors`. Sources: `skills/` (knowledge), `commands/roast.md`,
  the six roast agents, `codex-src/`.
...
- **Write discipline.** Filesystem mutation in `scripts/` routes through
  `scripts/lib/bridge.py`'s audited primitives (`tests/test_write_discipline.py` enforces
  by AST).
- **Inventory tables are cross-pinned.** The generator's source sets, vibe-check's
  `MIRROR_EXPECTED`, and the sweep's patterns each carry their own copy, and tests hold the
  copies identical — extend all of them together.
