...
//
// Spawns the launch exactly as `.codex/config.toml` will — S13 (vibe-214): `node <installed bin>`
// of the lockfile-verified generation, passed in as `<command> [args…]` — sends one MCP `initialize`
// over newline-delimited JSON-RPC, and requires a well-formed `serverInfo` back. Semver compliance
// does not imply the package boots on this machine, which is the whole point of verifying a pin.
//
// Two things this does NOT do, both deliberate:
//   * it does not kill only the direct child — a server may spawn descendants, so the process *group* is
...
//
// Seam: VIBE_SUITE_MCP_BIN replaces `<command>` for tests. The fake must *respond*; a probe test that
...
const [target, command, ...launchArgs] = process.argv.slice(2);
const TIMEOUT_MS = Number(process.env.VIBE_SUITE_PROBE_TIMEOUT_MS) || 30000;
const BIN = process.env.VIBE_SUITE_MCP_BIN || command;

if (!target || !command) {
  console.error("boot_probe: usage: boot_probe.mjs <package@version> <command> [args…]");
  process.exit(1);
}
...
const child = spawn(BIN, launchArgs, { stdio: ["pipe", "pipe", "pipe"], detached: true });
