...
<!-- GENERATED by scripts/mirror-sync.py from skills/agent-design/SKILL.md — do not edit. Slash commands refer to the Claude-side vibe-suite plugin; source paths refer to the vibe-suite repository. -->
...
- **Claude Code native subagents** (`.claude/agents/`, dispatched via the Task
  ...
  [writing-agents](../vibe-writing-agents/SKILL.md) for authoring them.
